...
const Carousel3D = ({
  ...
  }, [theta, anglePerCard, totalCards, currentIndex, transitionDuration, onRotate]);
  ...
  // Effect to handle current index and unflip cards when they're not centered
  useEffect(() => {
    if (currentIndex !== null && currentIndex !== undefined) {
      setFlippedCards(prev => {
        // Keep only the current card in the flip state
  ...
  return (
    ...
    >
                          ...
                          >
                            <span className="w-8 h-8 rounded-full bg-muted flex items-center justify-center">
                              <svg 
                                ...
                                strokeLinejoin="round"
                              >
                                <path d="M18 13v6a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h6m4-3h6v6m-11 5 5.586-5.586a2 2 0 0 1 2.828 0L21 15" />
                              </svg>
                            </span>
                            Ir al proyecto
                          </a>
      ...
      {/* Controls */}
      {showControls && (
        <div className="absolute bottom-6 left-1/2 transform -translate-x-1/2 flex items-center gap-4 z-10 top-[33rem]">
          <button 
            onClick={() => navigateCarousel(-1)} 
            className="w-10 h-10 rounded-full bg-background/80 border border-border text-foreground hover:bg-background/90 hover:scale-110 transition-all duration-200 flex items-center justify-center shadow-lg backdrop-blur-sm" 
